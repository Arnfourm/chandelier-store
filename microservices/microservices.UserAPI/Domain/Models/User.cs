...
namespace microservices.UserAPI.Domain.Models
{
    public class User
    {
        ...
        public User(Guid id, string email, string name, string surname, DateOnly? birthday, DateTime registration, Guid? passwordId, 
            Guid? refreshTokenId, UserRoleEnum userRole)
            : this(email, name, surname, birthday, registration, passwordId, refreshTokenId, userRole)
        {
            ValidateUserId(id);
            ...
            Guid? passwordId, Guid? refreshTokenId, UserRoleEnum userRole)
        {
            ...
            if (birthday == DateOnly.MinValue) throw new ArgumentOutOfRangeException("Birthday cannot be minimum date", nameof(birthday));
            if (birthday > DateOnly.FromDateTime(DateTime.Today)) throw new ArgumentOutOfRangeException("Birthday cannot be in the future", nameof(birthday));

            if (DateOnly.FromDateTime(DateTime.Today).AddYears(-150) > birthday) throw new ArgumentOutOfRangeException("Birthday seems unrealistic", nameof(birthday));

            if (registration == DateTime.MinValue) throw new ArgumentException("Registration date cannot be minimum date", nameof(registration));
            if (registration > DateTime.UtcNow.AddHours(1)) throw new ArgumentException("Registration date cannot be in the future", nameof(registration));
